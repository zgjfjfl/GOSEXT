...
local Heroes = {"Ornn", "JarvanIV", "Poppy", "Shyvana", "Trundle", "Rakan", "Belveth", "Nasus", "Singed", "Udyr", "Galio", "Yorick", "Ivern", 
				...
				"Briar", "Urgot", "Aurora", "Ambessa", "Mel", "Zaahen"}
...
function zgGalio:OnPreAttack(args)
	if Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_COMBO] then
...
function zgYorick:LaneClear()
	...
	local QARange = 225 + myHero.boundingRadius
	if IsValid(target) then
		if Menu.LaneClear.Q:Value() and IsReady(_Q) and lastQ + 300 < GetTickCount() and myHero:GetSpellData(_Q).name == "YorickQ" then
			if myHero.pos:DistanceTo(target.pos) < QARange + target.boundingRadius and self:getqDmg(target) >= target.health then
				Control.CastSpell(HK_Q)
				lastQ = GetTickCount()
				Control.Attack(target)
			end
...
function zgYorick:JungleClear()
	...
	if IsValid(target) then
		if Menu.JungleClear.Q:Value() and IsReady(_Q) and lastQ + 300 < GetTickCount() and myHero:GetSpellData(_Q).name == "YorickQ" then
			if myHero.pos:DistanceTo(target.pos) < QARange + target.boundingRadius then
				Control.CastSpell(HK_Q)
				lastQ = GetTickCount()
			end
		end
...
function zgAurelionSol:CastEAoE(target)
	if IsReady(_E) then
...
function zgAurelionSol:CastRAoE(target)
	if IsReady(_R) and lastR + 250 < GetTickCount() then
...
function zgZaahen:__init()
	...
	Orbwalker:OnPostAttack(function() self:OnPostAttack() end)
	WSpell = {Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.5, Radius = 35, Range = 850, Speed = 1600, Collision = false}
	ESpell = {Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0, Radius = 375, Range = 350, Speed = 900, Collision = false}
	RSpell = {Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 1.1, Radius = 550, Range = 600, Speed = MathHuge, Collision = false}
end
...
function zgZaahen:OnPostAttack()
	...
	if IsValid(target) and IsReady(_Q) and lastQ + 300 < GetTickCount() then
		if Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_COMBO] and target.type == Obj_AI_Hero then
			if Menu.Combo.Q:Value() and myHero:GetSpellData(_Q).name == "ZaahenQ" then
				Control.CastSpell(HK_Q)
				lastQ = GetTickCount()
			end
		end
		if Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_LANECLEAR] and Menu.Clear.SpellFarm:Value() and target.type == Obj_AI_Minion then
...
function zgZaahen:SemiManualR()
	if Menu.Combo.Rsm:Value() and IsReady(_R) then
		local mainTarget = TargetSelector:GetTarget(RSpell.Range + RSpell.Radius)
...
function zgZaahen:CastRWithMainTarget(mainTarget)
	local predictor = GGPrediction:SpellPrediction(RSpell)
	local aoeResults = predictor:GetAOEPrediction(myHero.pos)
...
function zgZaahen:Combo()
	...
	if Menu.Combo.E:Value() and IsReady(_E) and lastE + 300 < GetTickCount() then
		local target = TargetSelector:GetTarget(ESpell.Range + ESpell.Radius)
	...
	if Menu.Combo.W:Value() and IsReady(_W) then
		local target = TargetSelector:GetTarget(Menu.Combo.Wrange:Value())
		if IsValid(target) then
			self:CastW(target)
		end
	end
...
function zgZaahen:Harass()
	if Menu.Harass.W:Value() and IsReady(_W) then
		local target = TargetSelector:GetTarget(Menu.Harass.Wrange:Value())
		if IsValid(target) then
			self:CastW(target)
		end
	end
...
Callback.Add("Load", function()
	...
	local championIcon = "http://ddragon.leagueoflegends.com/cdn/15.23.1/img/champion/"..myHero.charName..".png"
