...
local SmiteDamage = 0;
local mySmiteSlot = 0;
--20+8*GetLevel(myHero) --getdmg("SMITE", target, myHero); --myHero.GetBuff(SmiteBuffNames).stacks; --GetSmiteDmg();
...
function Activator:LoadMenu()
	...
	self.Menu.ZS.MikaCC2:MenuElement({ id = "Asleep", name = "Use on Asleep", value = true})		
	...
	self.Menu.ZS.MikaCC:MenuElement({ id = "Asleep", name = "Use on Asleep", value = true})	
...
------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    self.Menu.summ.ex:MenuElement({id = "hp", name = "Self HP:", value = 30, min = 5, max = 95, identifier = "%"})
...
function Activator:Cleans2(unit)
	local CleanBuffs =
	{
		...
		[10]  = self.Menu.ZS.MikaCC2.Polymorph:Value(),			--Polymorph
		[11] = self.Menu.ZS.MikaCC2.Slow:Value(),				--Slow
		[12] = self.Menu.ZS.MikaCC2.Snare:Value(),				--Snare
		[22] = self.Menu.ZS.MikaCC2.Fear:Value(),				--Fear
		[23] = self.Menu.ZS.MikaCC2.Charm:Value(),				--Charm
		[35] = self.Menu.ZS.MikaCC2.Asleep:Value()				--Asleep
	}    
...
function Activator:Cleans(unit)
	local CleanBuffs =
	{
		...
		[10]  = self.Menu.ZS.MikaCC.Polymorph:Value(),			--Polymorph
		[11] = self.Menu.ZS.MikaCC.Slow:Value(),				--Slow
		[12] = self.Menu.ZS.MikaCC.Snare:Value(),				--Snare
		[22] = self.Menu.ZS.MikaCC.Fear:Value(),				--Fear
		[23] = self.Menu.ZS.MikaCC.Charm:Value(),				--Charm
		[25] = self.Menu.ZS.MikaCC.Suppression:Value(),			--Suppression
		[26] = self.Menu.ZS.MikaCC.Blind:Value(),				--Blind
		--[30] = self.Menu.ZS.MikaCC.KnockUp:Value(),			--KnockUp
		--[31] = self.Menu.ZS.MikaCC.KnockBack:Value(),			--KnockBack
		[32] = self.Menu.ZS.MikaCC.Disarm:Value(),				--Disarm
		[35] = self.Menu.ZS.MikaCC.Asleep:Value()				--Asleep
	}    
...
local function GetSmiteDamage(unit, mySmiteSlot)
	if myHero:GetSpellData(mySmiteSlot).name == "SummonerSmite" then
        return 450
    elseif myHero:GetSpellData(mySmiteSlot).name == "S5_SummonerSmiteDuel" then
        return 900
    elseif myHero:GetSpellData(mySmiteSlot).name == "S5_SummonerSmitePlayerGanker" then
        return 900
    elseif unit.type == Obj_AI_Hero then
		if myHero:GetSpellData(mySmiteSlot).name == "s5_summonersmiteplayerganker" then
		  return 20+8*myHero.levelData.lvl
		elseif myHero:GetSpellData(mySmiteSlot).name == "s5_summonersmiteduel" then
		  return 48+77/17*(myHero.levelData.lvl-1)
		end
    	return 0
    end
end
...
if myHero.alive == false then return end 
	...
	if (myPotTicks + 1000 < GetTickCount()) and self.Menu.Healing.Enabled:Value() then
		...
		for i = 0, 63 do
			...
			--print(buffData.name)
				if (buffData.type == 14) or (buffData.type == 27) then 
	...
	for i, target in pairs(GetEnemyHeroes()) do
		...
		if target and GetDistance(myHero.pos, target.pos) < 2000 and IsValid(target) then
			...
			if self.Menu.summ.ex.target:Value() then
				if myHero.pos:DistanceTo(target.pos) <= 650 and myHero.health/myHero.maxHealth <= self.Menu.summ.ex.hp:Value()/100 then
